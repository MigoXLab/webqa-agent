...
def build_web_agent_system_prompt(
    ...
    *,
    ...
    base = (
        ...
        ' Then continue.\n\n'
        '### Recovery Protocol\n'
        'When verification fails (outcome diverges from expectation) or a '
        'tool returns an error, follow this sequence:\n\n'
        '1. **Re-observe** — batch `take_snapshot` + `take_screenshot` to '
        'get ground truth. Compare against the state *before* the failed '
        'action: what changed? What didn\'t? Any new blockers (modals, '
        'errors)?\n'
        '2. **Diagnose** — classify the failure:\n'
        '   - Execution error (tool reported failure) vs semantic error '
        '(tool succeeded, wrong effect).\n'
        '   - Assess progress: partial (preserve what worked), zero '
        '(approach may be wrong), or negative (undo first).\n'
        '3. **Recover** — escalation ladder (try in order, escalate on '
        'failure):\n'
        '   - Retry with modification (alternative selector, corrected '
        'input, wait_for before retry).\n'
        '   - Alternative tool (`type_text` instead of `fill`, adjust '
        '`cdp_upload_file` selector, `evaluate_script` for direct DOM '
        'manipulation).\n'
        '   - Replan (different path to the same goal: direct URL, '
        'different entry point).\n'
        '   - Skip and record (log what failed, what was tried, move on).\n'
        '4. After recovery, **re-observe** to verify the fix worked '
        'before continuing.\n\n'
