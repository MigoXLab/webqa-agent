...
def build_web_agent_system_prompt(
    ...
    *,
    ...
    base = (
        ...
        '2. **Narrate** — write one short Chinese sentence describing what '
        'you are about to do (e.g. "点击「提交」按钮，提交表单。").\n'
        '3. **Act + Screenshot** — STRICT RULE: every time you call a '
        'mutating tool (click, fill, navigate_page, wait_for, press_key, '
        'hover, drag, upload_file, select_option), you MUST include '
        '`take_screenshot` in the exact same response batch. '
        'NEVER call a mutating tool alone without `take_screenshot`. '
        '❌ Wrong: one turn with only `click`. '
        '✅ Correct: one turn with `click` + `take_screenshot` together.\n'
