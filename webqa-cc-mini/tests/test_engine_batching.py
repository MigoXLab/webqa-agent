...
def test_tool_base_class_default_concurrent_safe_is_true() -> None:
    class _T(Tool):
        ...
        def execute(self, **_kwargs: Any) -> ToolResult: return ToolResult(content='')
...
def _drive_engine(engine: Engine, tool_uses: list[dict[str, Any]]) -> list[tuple]:
    """Run one assistant turn that emits ``tool_uses``, then a second turn
    that finishes with text — captures every event."""
    streams = [
...
def test_heavy_tool_runs_in_singleton_sequential_batch() -> None:
    ...
    with patch('core.engine.ThreadPoolExecutor', _RecordingPool):
...
def test_verify_tool_is_not_concurrent_safe() -> None:
    ...
    sibling reads queues everything on the same renderer thread.  Class-level
    flag, no instance construction needed."""
...
class _WaitForCapturingTool(Tool):
    ...
    receives.  Lets us assert what the engine actually forwarded after the
    clamp ran, without spinning up chrome-devtools-mcp."""
...
def test_wait_for_clamp_zero_falls_back_to_default() -> None:
    """chrome-devtools-mcp treats ``timeout=0`` as "use server default",
    which lets an unmet condition stall the renderer past our outer 60s
    call timeout.  The clamp replaces 0 with the short default so the
    renderer is released long before the outer clock fires."""
...
def test_wait_for_clamp_over_max_is_capped() -> None:
    """Anything above ``_WAIT_FOR_MAX_TIMEOUT_MS`` is capped so the cdm
    retry tail plus the cancel/response round-trip on stdio still has
    headroom before the outer ``_DEFAULT_CALL_TIMEOUT`` fires.  The cap
    also bounds wasted wait time when the requested text never matches
...
def test_stateful_read_only_tool_runs_sequentially(tmp_path: Path) -> None:
    ...
    invocations would otherwise be co-batched with cheap reads, it must
    drop into sequential mode."""
