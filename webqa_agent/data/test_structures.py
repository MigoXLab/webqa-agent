...
class ParallelCaseExecutionConfig(BaseModel):
    """Configuration for parallel case execution.

    This configuration controls how test cases are executed in parallel
    after being generated by the planner.
    """

    enabled: bool = False  # Enable/disable parallel case execution
    max_concurrent_cases: int = 2  # Maximum number of cases running concurrently
    session_pool_size: Optional[int] = None  # Browser session pool size (defaults to max_concurrent_cases)

    def get_session_pool_size(self) -> int:
        """Get the session pool size, defaulting to max_concurrent_cases if not specified."""
        return self.session_pool_size if self.session_pool_size is not None else self.max_concurrent_cases
