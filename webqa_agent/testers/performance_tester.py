...
from webqa_agent.data.test_structures import SubTestResult, SubTestReport
import logging
...
class LighthouseMetricsTest:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                if process.returncode != 0:
                    ...
                    if "Cannot find package 'lighthouse'" in stderr_output or "Cannot find module" in stderr_output and "lighthouse" in stderr_output:
                        raise Exception("Lighthouse npm package is not installed. Please run: npm install lighthouse")
                    elif "Cannot find package 'chrome-launcher'" in stderr_output or "Cannot find module" in stderr_output and "chrome-launcher" in stderr_output:
                        raise Exception(
    ...
    def extract_ai_optimized_performance_data(self, lhr):
        ...
        performance_score = round(performance_category.get('score', 0) * 100) if performance_category.get(
        ...
        for audit_id, metric_info in metrics_to_extract.items():
            if audit_id in audits:
                ...
                if metric_info['is_core_vital']:
                    performance_metrics[metric_info['id']][
                        'passes_threshold'] = score >= 0.9 if score is not None else False
        ...
        category_scores = {
            ...
            'accessibility': round(accessibility_category.get('score', 0) * 100) if accessibility_category.get(
                'score') is not None else 0,
            'best_practices': round(best_practices_category.get('score', 0) * 100) if best_practices_category.get(
                'score') is not None else 0,
        ...
        perf_metrics_str = "\n".join([
            f"- {m['name']}: {m['display_value']}" for m in performance_metrics.values()
        ])
        simple_report.append({
            "title": "性能指标",
            "issues": perf_metrics_str
        })
    ...
    def _extract_minimal_page_stats(lhr):
        """Extract simplified page statistics, only used for generating recommendations"""
        audits = lhr.get('audits', {})
        minimal_stats = {
            'total_size_kb': 0,
            'total_requests': 0,
    ...
    def _extract_seo_issue_details(audit_id, details):
        ...
        if audit_id == 'document-title':
            ...
            if details.get('items'):
                ...
                extracted_details['problematic_links'] = [item.get('text', '') for item in
                                                          details.get('items', [])[:5]]  # 最多显示5个
    ...
    def _generate_recommendations(core_vitals, opportunities, diagnostics, page_stats, seo_issues):
        ...
        seo_issues_sorted = sorted(seo_issues, key=lambda x: {
            'critical': 4, 'serious': 3, 'moderate': 2, 'minor': 1
        }.get(x.get('impact'), 0), reverse=True)
