...
The feature's raison d'être is that N parallel ``run_cc_mini`` calls, each
...
def test_concurrent_workers_do_not_cross_contaminate_cookies(patched):
    ...
    t1 = threading.Thread(target=_run_worker, args=(1, _VIEWER),
                          name='worker-1')
    t0.start(); t1.start()
    t0.join(timeout=10.0); t1.join(timeout=10.0)
...
def test_worker_cookies_match_worker_id(patched):
    """For any worker_id, the injected cookies are that worker's, on that worker's port."""
