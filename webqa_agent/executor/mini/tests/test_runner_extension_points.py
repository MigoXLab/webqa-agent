...
class _FakeEngine:
    ...
    Captures the system prompt + tools for assertions, yields zero events
    from submit() so run_cc_mini returns immediately with empty steps.
    """
...
def patched_runner(monkeypatch):
    """Patch MCPManager + Engine + CompactService to isolate extension-point logic."""
    ...
    class _NoopCompact:
        ...
        def compact(self, messages, system_prompt): return (messages, 0)
...
class _FakeConfig:
    ...
    attribute reads; we avoid importing the real dataclass so the tests
    don't break on signature additions."""
...
class TestResolveCdpPort:
    ...
    def test_bare_remote_debugging_port_ignored(self):
        """Bare --remote-debugging-port is silently dropped by chrome-devtools-mcp;
        we correctly don't trust it."""
        cfg = _FakeConfig('browser', (
...
def test_custom_mcp_without_resolvable_port_records_extension_failure(patched_runner):
    ...
    def _fake_manager_factory(_configs):
        class _FakeMCP:
            def __init__(self):
                self._servers = {'browser': server}
            def start_and_collect_tools(self): return []
