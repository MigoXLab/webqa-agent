...
def iso_from_monotonic(end_iso: str, duration_seconds: float) -> str:
    """Derive the start timestamp by subtracting *duration_seconds* from *end_iso*.
...
def usage_payload(usage: Any) -> dict[str, int]:
    """Normalise an LLM usage object into a plain dict with canonical token fields."""
...
def safe_copy(value: Any) -> Any:
    """Return a deep copy of *value*, falling back to ``str(value)`` on error."""
    try:
...
def build_tool_event_payload(
    *,
    ...
    """Build the payload dict for a ``cc_mini_tool_call`` or ``cc_mini_tool_result`` event.
